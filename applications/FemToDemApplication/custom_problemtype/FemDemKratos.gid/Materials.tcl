...
proc WriteMaterials { basename dir problemtypedir TableDict} {
    ...
    puts $FileVar "KratosMultiphysics.SolidMechanicsApplication import *"
    puts $FileVar "KratosMultiphysics.FemToDemApplication import *"
    puts $FileVar ""
    ...
    for {set i 0} {$i < [llength $Groups]} {incr i} {
        ...
        #set ConstLaw [GiD_AccessValue get gendata Plane_state]

        if {[GiD_AccessValue get gendata Plane_state] eq "Plane_Strain"} {

        	puts $FileVar "    mat = LinearElasticPlaneStrain2DLaw()"

        } else {

        	puts $FileVar "    mat = LinearElasticPlaneStress2DLaw()"
        }
