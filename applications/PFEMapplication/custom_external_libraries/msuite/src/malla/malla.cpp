...
//  else if (strlen(ext)==1&&ext[0]=='e') {if (!graba_e(filename)) {_savetime(error);return false;}}
    ...
    {
        add_error(Unknown_Extension);
        add_error(ext);
        if (!graba_dat(filename))
        {
            _savetime(error);
            return false;
        }
...
bool malla::graba_frontera()
{
    ...
    for (int i=0; i<frontera.len; i++)
    {
        ...
        if(!(frontera[i].graba_dat(fnombre)))
        {
            _savetime(error);
            return false;
        }
...
bool malla::lee(const char* const_inombre)
{
    ...
    else   // usa el que ya tiene
    {
        ...
        else
        {
            ...
            strcat(inombre,".");
            if (ext[0]) strcat(inombre,ext);
            else strcat(inombre,"dat");
        }
    }
    ...
    array1<nodo> nl;
    array1<elemento> el;
    if (!strcmp_nocase(ext,"xyz"))
    {
        if (!lee_xyz(nombre,nl))
        {
            _savetime(error);
            return false;
        }
    }
    else if (!strcmp_nocase(ext,"xy"))
    {
        if (!lee_xy(nombre,nl))
        {
            _savetime(error);
    ...
    else if (strlen(ext)==1&&ext[0]=='e')
    {
        if (!lee_e(nombre,nl,el))
        {
            _savetime(error);
            return false;
        }
    ...
    if (el)
    {
        ...
        for (i=0; i<el.len; i++)
        {
            ...
            if (j<nv)
            {
                ...
                {
                    elemento e3(e_wedge);
                    for (j=0,nv=0; j<4; j++) if (ei[j]!=ei.npos(j))
                        {
                            e3[nv]=ei[j];
                            e3[nv+3]=ei[j+4];
                            nv++;
...
malla& malla::operator +=(malla &m) // suma de mallas
{
    ...
    {
        hayv=true;
        set_min(nvmin,m.nvmin);
        set_max(nvmax,m.nvmax);
    }
    else
    {
        hayv=false;
    ...
    for (i=0; i<m.e.len; i++) // b es const
    {
        ...
        in=ei[0];
        rep=false;
        if (in<oldnlen)  // si no es un nodo nuevo
        {
...
bool malla::extrude(const punto &v)
{
    ...
    for (i=0; i<nlen; i++)
    {
        p=n[i]+v;
        n+=n[i];
        n.last().setpos(p); // mismo h, flag y valor
    }
