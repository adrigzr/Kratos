...
static int mk_mfija(malla &m, flagtype flag_fija)
{
    ...
    // elementos
    pline ix(int(1.2*m.e.len));
    cfija.e.resize(int(1.2*m.e.len)); // preasigna espacio
    m.mask_elms(&cfija.e,&ix,flag_fija);
    // malla fija
    if (mfija) delete mfija;
    mfija=new malla(cfija);
    efijos=mfija->e.len;
    // frontera de la fija o intefase
    mfija->mk_vecino();
    mfija->graba_dat();
    if (nf_fijo) delete nf_fijo;
    nf_fijo=new pline;
    pline &nff=*nf_fijo;
    for (i=0; i<mfija->n.len; i++)
    {
        if (mfija->n[i].f.es(n_frontera)) nff+=i;
    }
...
static int recover_elms(malla &m, flagtype flag_fija)
{
    ...
    for (i=0; i<nflen; i++)
    {
        ...
        for (j=0; j<qei; j++)
        {
            ...
            if (!m.diagonal_swap(ie,ieo,false))
                return 0;
            i--;
            break;//testea de nuevo el mismo nodo
        }
    }

    // recupera interior de la malla solida
    pline ix(efijos);
    m.mask_elms(0,&ix,flag_fija);
    ...
    array1<elemento>e(mfija->e);
    e.resize(efijos+m.e.len-ix.len);
