...
poly_dat::poly_dat(const poly_dat &p)
{
    ...
    if (p.cn)
    {
        ...
        for (i=0; i<nv; i++)
        {
            nci=p.cn[i][0]+1;
            cn[i]=new int[nci];
            memcpy(cn[i],p.cn[i],nci*SZI);
        }
    }
    else cn=0;

    t=new int[NV*nt];
    memcpy(t,p.t,NV*nt*SZI);
}
...
{
    int i=nv(),j=e.nv();
    if (i<j||(strict&&i==j)) return false;
...
elemento elemento::cara(int i) const
{
    ...
    else if (Tipo==e_wedge)
    {
        ...
        if (i==0)
        {
            ec.n[0]=1;    // 0: cuadrilatero opuesto al 0
            ec.n[1]=2;
            ec.n[2]=5;
            ec.n[3]=4;
        }
        ...
        {
            ec.n[0]=2;    // 1: cuadrilatero opuesto al 1
            ec.n[1]=0;
            ec.n[2]=3;
            ec.n[3]=5;
        }
        ...
        {
            ec.n[0]=0;    // 2: cuadrilatero opuesto al 2
            ec.n[1]=1;
            ec.n[2]=4;
            ec.n[3]=3;
        }
        ...
        {
            et.n[0]=0;    // 3: triangulo inferior
            et.n[1]=2;
            et.n[2]=1;
        }
        else if (i==4)
        {
            et.n[0]=3;    // 4: triangulo superior
            et.n[1]=4;
            et.n[2]=5;
        }
        if (i<3)
...
int elemento::icara(const elemento &c) const
{
    ...
    while (--i>=0)
    {
        ...
        j=nvc;
        while (--j>=0) if (!ci.have(c[j])) break;
        if (j<0) break;
    }
    return i;
}
...
void elemento::cara(int i, int &nvc, int *ix) const
{
    ...
    else if (Tipo==e_wedge)
    {
        ...
        if (i==0)
        {
            ix[0]=1;    // 0: cuadrilatero opuesto al 0
            ix[1]=2;
            ix[2]=5;
            ix[3]=4;
        }
        ...
        {
            ix[0]=2;    // 1: cuadrilatero opuesto al 0
            ix[1]=0;
            ix[2]=3;
            ix[3]=5;
        }
        ...
        {
            ix[0]=0;    // 2: cuadrilatero opuesto al 0
            ix[1]=1;
            ix[2]=4;
            ix[3]=3;
        }
        ...
        {
            ix[0]=0;    // 3: triangulo inferior
            ix[1]=2;
            ix[2]=1;
        }
        else if (i==4)
        {
            ix[0]=3;    // 4: triangulo superior
            ix[1]=4;
            ix[2]=5;
        }
        if (i<3) nvc=4;
...
void elemento::lee(const int *linea)
{
    ...
    else if (Tipo==e_poligono)
    {
        ...
        pd->nv=pd->nc=NV;
        n=new int[NV];
        memcpy(n,&linea[1],NV*SZI);
        pd->nt=NT=NV-2;
        NT*=3;
        pd->t=new int[NT];
        memcpy(pd->t,&linea[1+NV],NT*SZI);
    }
    else if (Tipo==e_poliedro)
    {
        if (pd) delete pd;
        ...
        NC*=3;
        pd->c=new int[NC];
        memcpy(pd->c,&linea[2+NV],NC*SZI);
        NT=linea[2+NV+NC];
        pd->nt=NT;
        NT*=4;
        pd->t=new int[NT];
        memcpy(pd->t,&linea[3+NV+NC],NT*SZI);
    }
    else
    {
        NV=tnv[Tipo];
        memcpy(n,linea,NV*SZI);
    }
    if (iobase)
    {
        int i=0;
...
void poly_dat::rm_cn()
{
    if (cn)
    {
        for (int i=0; i<nv; i++) delete [] cn[i];
        delete[] cn;
        cn=0;
    }
}
...
//      3,4,6,7,
    ...
    pd->c=new int[36];
    memcpy(pd->c,c,36*SZI);
    pd->t=new int[24];
    memcpy(pd->t,t,24*SZI);
    Tipo=e_poliedro;
    return *this;
}
...
void poly_dat::mk_cn()
{
    ...
    for (i=0; i<nv; i++)
    {
        ...
        for (j=1; j<qc; j++)
        {
            ic=cn[i][j];
            if (i==c[3*ic]) iv=2;
            else if (i==c[3*ic+1]) iv=0;
            else iv=1; // el anterior
