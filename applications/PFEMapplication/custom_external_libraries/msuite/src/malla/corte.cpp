...
static void agrega_n(double alpha)
{
    ...
    if (alpha<ERRADM) alpha=0;
    else if(alpha>1-ERRADM) alpha=1;
    int q,nc, in=n+=nodo(m.n[nodo_inicio],m.n[nodo_final],alpha);
    ne[qne]=in;
    qne++; //avisa al elemento que tiene este nodo
...
static void cut_tetras()
{
    ...
    malla &corte=*mc;
    malla &m=*mm;
    pline& eplane=*_eplane;
    array1<nodo> &n=corte.n;
    array1<elemento> &e=corte.e;
        ...
        for(j=0; j<nt; j++) // para cada tetraedro del elemento
        {
            ...
            */
            ...
            if(dif1)
            {
                ...
                {
                    _nf(2);
                    mk_intersection(); // 01 y 02
                    if(dif3)
                    {
                        _nf(3);
                        mk_intersection(); // 01, 02 y 03 => A
                    }
                    else  // 01 y 02 pero no 03 ==> G
                    {
                        _ni(3);
                        mk_intersection(); // corta 32
                        _nf(1);
                        mk_intersection(); // corta 31
                    }
                }
                else if(dif3)
                {
                    _ni(2);
                    mk_intersection(); // corta 21
                    _nf(3);
                    mk_intersection(); // corta 23
                    _ni(0);
                    mk_intersection(); // 01 y 03 pero no 02 => F
                }
            ...
            {
                _nf(2);
                mk_intersection(); // no corta 01 pero corta 02
                if(dif3)
                {
                    _nf(3);
                    mk_intersection(); // corta 03 => E
                    _ni(1);
                    mk_intersection(); // corta 13
                    _nf(2);
                    mk_intersection(); // corta 12
                }
            ...
            {
                _nf(3);
                mk_intersection(); // no corta 01 ni corta 02 pero corta 03 => D
                _ni(1);
                mk_intersection(); // corta 13
                _ni(2);
                mk_intersection(); // corta 23
