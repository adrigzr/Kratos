...
//if (odbg) cout << "p2n: " << p2n << endl;
    ...
    p2h=(p00[2]-av*p2n)-c;
    l=p2h.mod(); // l=distancia al centro
...
bool malla::es_sliver(int ie, double h) const
{
    ...
    punto p[3]= {n[ei[1]]-n[ei[0]],n[ei[2]]-n[ei[0]],n[ei[3]]-n[ei[0]]};
    p00=p;
...
//if (odbg) cout << "hc3*_volchico/6: " << hc*hc*hc*_volchico/6 << endl;
    if (si.vt>hc*hc*hc*_volchico/6)
    {
        si.f.reset(e_sliver);    // vol grande
        return false;
    }
    punto p[3]= {n[si[1]]-n[si[0]],n[si[2]]-n[si[0]],n[si[3]]-n[si[0]]};
    p00=p;
    if (!_es_sliver())
    {
        si.f.reset(e_sliver);
...
//if (odbg) cout << "ES SLIVER" << endl;
    ...
    if (imaxd0!=0)
    {
        si.swap(0,imaxd0);    // imaxd0 pasa a ser el nodo 0
        nswaps++;
    }
    if (imaxd1!=2)
    {
        si.swap(2,imaxd1);    // imaxd1 pasa a ser el nodo 2
        nswaps++;
    }
    if (nswaps==1)
    {
        si.swap(1,3);   // hay que hacer otro para que quede derecho
    }
    return true;
}
...
bool malla::mark_sliver(int ie)
{
    ...
    if (ve&&ve[ie]>hc*hc*hc*_volchico/6)
    {
        ei.f.reset(e_sliver);    // vol grande
        return false;
    }
    punto p[3]= {n[ei[1]]-n[ei[0]],n[ei[2]]-n[ei[0]],n[ei[3]]-n[ei[0]]};
    p00=p;
    ...
    {
        Swap(ne[0],ne[imaxd0]);
        if (vecino) Swap(vecino[ie][0],vecino[ie][imaxd0]);
        swaps++;
    }
    if (imaxd1!=2) // imaxd1 pasa a ser el nodo 2
    {
        Swap(ne[2],ne[imaxd1]);
        if (vecino) Swap(vecino[ie][2],vecino[ie][imaxd1]);
        swaps++;
    }
    if (swaps==1) // hay que hacer otro
    {
        Swap(ne[1],ne[3]);
        if (vecino) Swap(vecino[ie][1],vecino[ie][3]);
    }
    return true;
}
...
void voronoi::rm_i_slivers()
{
    ...
    while (rehacer)
    {
        ...
        if (rehacer.len==oldilen&&rehacer_f_slivers.len==oldflen) count++;
        else count=0;
...
bool voronoi::rm_i_sliver(int is)
{
    ...
    si.swap(0,1);
    si.swap(2,3);// se labura por la arista 0-2
    if (swap4x4(is)) return true;
    rehacer_i_slivers+=is;
    return false;
}
...
//    s[is0].f.set(flag3); s[is1].f.set(flag3); s[is2].f.set(flag3);
    ...
    si.vecino[1]=vi2;
    si.vecino[2]=vi1;
    si.vecino[3]=is1; // orientacion
    if (vs0>=0) s[vs0].replace_vecino(is0,is1);
    if (vs2>=0) s[vs2].replace_vecino(is2,is1);
    if (vi0>=0) s[vi0].replace_vecino(is0,is2);
    if (vi1>=0) s[vi1].replace_vecino(is1,is2);
    ...
    n[ins].e.remove1(is0);
    n[ins].e.remove1(is2); // ns pierde a s0 y a s2
    n[ini].e.remove1(is0);
    n[ini].e.remove1(is1); // ni pierde a s0 y a s1
    ...
    s[is2]=si; // reemplaza
    _retest_s(is1,fs);
    _retest_s(is2,fi); // ojo que mark_sliver puede cambiar la orientacion
    ...
    {
        isl=s.len-1;
        while (s[isl].f.es_alguno(e_sliver|e_borrado)&&isl>=0)
        {
            isl--;
        }
        _revienta(isl<0);
        ...
        for (j=0; j<4; j++)
        {
            ...
            v=sl.vecino[j];
            if (v>=0) s[v].replace_vecino(isl,is0);
        }
        s[is0]=sl;
        sl.f.set(e_borrado);
        s.remove(isl); // por si isl no es la ultima
    }
    else s.len--; // s0 era la ultima
    while (sb.len&&sb.last()==s.len-1)
    {
        sb.len--;    // por si es la ultima
        s.len--;
    }
...
bool voronoi::swap4x4(int is0)
{
    ...
    {
        is1=se[k];
        if (is1!=is0&&is1!=is2&&is1!=is3) break;
    }
    ...
    k=s1.index(in0);
    iv3=s1.vecino[k];
    k=s2.index_vecino(is0);
    in4=s2[k];
    k=s2.index(in2);
    iv4=s2.vecino[k];
    k=s2.index(in0);
    iv5=s2.vecino[k];
    k=s3.index_vecino(is0);
    in5=s3[k];
    k=s3.index(in2);
    iv6=s3.vecino[k];
    k=s3.index(in0);
    ...
    if (primerpaso && ((iv2>=0&&iv2==iv4)||(iv3>=0&&iv3==iv5)))
    {
        ...
        s[is0].swap(0,2);
        s[is0].swap(1,3); // la otra diagonal
        primerpaso=false;
    ...
    s0.define();
    s0.f=f;
    if (iv6<0||iv1<0) s0.f.set(e_frontera);
    ...
    s1.define();
    s1.f=f;
    if (iv7<0||iv0<0) s1.f.set(e_frontera);
    ...
    s2.define();
    s2.f=f;
    if (iv2<0||iv4<0) s2.f.set(e_frontera);
    ...
    s3.define();
    s3.f=f;
    if (iv3<0||iv5<0) s3.f.set(e_frontera);
    ...
    if (iv2>=0) s[iv2].replace_vecino(is1,is2);
    if (iv6>=0) s[iv6].replace_vecino(is3,is0);
//  if (iv3>=0) s[iv3].replace_vecino(is1,is3); if (iv7>=0) s[iv7].replace_vecino(is3,is1);
    if (iv3>=0) s[iv3].replace_vecino(is1,-s.len);
    if (iv7>=0) s[iv7].replace_vecino(is3,is1);
    if (iv3>=0) s[iv3].replace_vecino(-s.len,is3);
    ...
    while (rehacer)
    {
        ...
        */
        ...
        if (is!=s.len-1)
        {
            ...
            s[is]=sl;
            sl.f.set(e_borrado);
            s.remove(isl); // por si no es la ultima
        }
        else s.len--;
        while (sb.len&&sb.last()==s.len-1)
        {
        ...
        for(k=0; k<4; k++)
        {
            ...
            for(j=0; j<eni.len; j++)
            {
                ...
                if (ve[iej]>h3*_volchico/6)  // arreglado
                {
                    ...
                    // lo saca de la lista
                    rehacer[rehacer.index(iej)]=rehacer.last();
                    rehacer.len--;
                    ej.f.reset(e_sliver);
                }
                else
