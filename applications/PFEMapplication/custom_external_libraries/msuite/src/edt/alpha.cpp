...
//  for (i=0;i<n.len;i++) n[i].f.reset(n_frontera); !!no, ya hay frontera
    ...
    for (i=0; i<s.len; i++)
    {
        ...
        const int *nei=ei.n;
        for (hi=n[nei[0]].h,j=1; j<NV; j++) hi+=n[nei[j]].h;
        hi/=NV; // h medio
//    for (hi=n[nei[0]].h,j=1;j<NV;j++) set_min(hi,n[nei[j]].h);// h min
        r=ei.r;
        if (r==0) rc=MAXREAL;
        else rc=r/hi; // (0=>colineales o coplanares => vuela)
...
void voronoi::alpha_shape_carving(double alpha, bool nodelete)
{
    ...
    for (i=0; i<s.len; i++) if (s[i].f.es(e_frontera))
        {
            rehacer+=i;
            s[i].f.set(flag1);
        }
...
//    for (hi=n[nei[0]].h,j=1;j<NV;j++) set_min(hi,n[nei[j]].h);// h min
        ...
        if (nodelete)
        {
            ...
            for (j=0; j<NV; j++)
            {
                ...
                {
                    s[v].f.set(e_frontera);
                    if (s[v].f.noes_ninguno(flag1|flag2))
                    {
                        rehacer+=v;
                        s[v].f.set(flag1);
                    }
...
void malla::alpha_shape_carving(double alpha)
{
    ...
    // presupongo frontera marcada en elementos => e_frontera -> rehacer
    for (i=0; i<e.len; i++) if (e[i].f.es(e_frontera))
        {
            rehacer+=i;
            e[i].f.set(flag1);
        }
